...
if (!isset($_SESSION['id_responsa']) && !isset($_SESSION['id_responsavel'])) {
    returnError('Usuário não autenticado');
}
...
$id_responsavel = $_SESSION['id_responsa'] ?? $_SESSION['id_responsavel'] ?? null;

if (!$id_responsavel) {
    returnError('ID do responsável não encontrado');
}

try {
    // ============================================
    // BUSCAR TODOS OS NEURODIVERGENTES
    // 1. Cadastrados diretamente (id_responsa)
    // 2. Vinculados através da tabela de vínculos
    // ============================================
    ...
    $stmt = $sql->prepare("
        SELECT DISTINCT
            ...
            n.perfil,
            n.data_cadastro,
            r_original.nome as nome_responsavel_original,
            r_original.celular as celular_responsavel_original,
            CASE 
                WHEN n.id_responsa = ? THEN 'principal'
                ELSE 'secundario'
            END as tipo_vinculo
        FROM cad_neurodivergentes n
        INNER JOIN cad_responsavel r_original ON n.id_responsa = r_original.id_responsa
    ...
    while ($dados = $result->fetch_assoc()) {
        ...
        $dados_formatados = [
            ...
            'nome_responsavel' => $dados['nome_responsavel_original'],
            'celular_responsavel' => $dados['celular_responsavel_original'],
        ...
        if (!empty($dados['celular_responsavel_original'])) {
            $cel_resp = preg_replace('/\D/', '', $dados['celular_responsavel_original']);
            if (strlen($cel_resp) === 11) {
                $dados_formatados['celular_responsavel_formatado'] = '(' . substr($cel_resp, 0, 2) . ') ' . 
                                                                       substr($cel_resp, 2, 5) . '-' . 
                                                                       substr($cel_resp, 7, 4);
            } else {
                $dados_formatados['celular_responsavel_formatado'] = $dados['celular_responsavel_original'];
            }
        } else {
